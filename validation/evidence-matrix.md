...
| `VAL-007` | Bed bug prep / follow-up path | `route add-stop`, `closeout prep`, `closeout note`, `closeout evaluate` | `0` or `2` depending on follow-up vs blocked state | `prep_notice.required` or `prep_notice.cleared`, `followup.scheduled`, possible `job.closeout.blocked` | optional media | Pass if prep failure or missing follow-up prevents false clean closeout | not_yet_proven |
...
| `VAL-009` | Missed check-in recovery | `route update-stop`, `report end-day` | `0`, report success codes | `field_checkin.received` equivalent route-stop update event | none required | Pass if unresolved work is preserved and no silent closeout or billing occurs | degraded_acceptable |
...
These are backed by existing CLI integration tests and should still be recorded through the run-report process when future agents validate the full playbook.

## Degraded But Acceptable

- voice memo intake without transcript engine
- missed check-in recovery through unresolved/review handling
...
- bed bug workflow depth beyond the minimum prep/follow-up protection path
